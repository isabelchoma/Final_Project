...
# Train/test split
set.seed(123)
split_idx <- createDataPartition(hitters_data$Logged_Salary, p = 0.8, list = FALSE)
hitters_data2 <- hitters_data
colnames(hitters_data2)[4]="Games_Played"
colnames(hitters_data2)[6]="Birth_Year"
train_data <- hitters_data2[split_idx, ]
test_data <- hitters_data2[-split_idx, ]
...
# Evaluate
lm_results <- postResample(lm_preds, test_data$Logged_Salary)
print("Linear Regression Results:")
print(lm_results)

```

Our model had low explanatory power, with an R-squared of 0.08878462, which means that the model explains only 8.87% of the variance in Logged_Salary.
...
### RANDOM FOREST
rf_model <- randomForest(formula, data = train_data, ntree = 1000, mtry = 4, maxnodes = 30, importance = TRUE)

# Predict
rf_preds <- predict(rf_model, newdata = test_data)

# Evaluate
rf_results <- postResample(rf_preds, test_data$Logged_Salary)
...
R-squared improved to 0.1923417, meaning the model explains 19.23% of variance in Logged_Salary. 

RMSE increased to 1.2332885, meaning he model’s predictions are now off by about 1.23 log-salary units on average

MAE decreased to 1.0202420, meaning now the average absolute prediction error is about 1.02 log-salary units

The model is still under-fitting. 

We wanted to plot the model to see how it fits the data and how much it is under-fitting.

```{r, echo=FALSE}

#Plot representing underfitting or overfitting
ggplot(data = NULL, aes(x = test_data$Logged_Salary, y = rf_preds)) +
  geom_point(color = "blue") +
  geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed") +
  labs(title = "Random Forest: Predicted vs Actual Logged Salary",
...
Predictive Power: Our best model explained approximately 72% of salary variance, demonstrating that while performance metrics and player characteristics strongly influence compensation, other factors not captured in our dataset (such as market size, team budget constraints, or negotiation effectiveness) also play significant roles.
